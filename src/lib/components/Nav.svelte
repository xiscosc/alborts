...
async function scrollTo(e: MouseEvent) {
	...
	function scrollToElement(el: HTMLElement, behavior: ScrollBehavior) {
		...
		const offset = navHeight + 16; // nav height + small buffer
		window.scrollTo({ top: el.offsetTop - offset, behavior });
	}
...
>
	<ul class="flex items-center justify-center gap-1 px-2 py-2 md:gap-2 md:px-4 md:py-2">
		<li><a
			href="/#home"
			onclick={scrollTo}
